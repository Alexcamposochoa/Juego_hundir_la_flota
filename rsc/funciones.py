...
def bienvenida():
    print(
        "Hola grumete!! Bienvenido a tu primera práctica en la flota del general Miguel Pata de palo, yo soy el comandante general de su flota, me llaman Capitán Pytón..."
    )  # time.sleep(2)
    print("...       ...       ...")
    time.sleep(4)
    print(
        "Para el día de hoy tenemos una misión sencilla, tendrás que destruir todos los barcos de la flota de nuestros temibles enemigos 'La banda de los TA's malvados'"
    )
    time.sleep(4)
    print("...       ...       ...")
    time.sleep(4)
    ...
    print(f"Muy bien!{nombre} espero que estés preparad@ para lo que te espera")
    time.sleep(4)
    ...
    time.sleep(2)
    print(
        "La dinámica es sencilla, se colocarán los barcos de manera aleatoria y tú comenzarás disparando en la coordenada que creas que se han escondido estos granujas."
    )
    time.sleep(4)
    print(
        "Por ejemplo: Coordenada 0 del eje X , coordenada 5 del eje Y!!! DISPAREEEEN!!!MUahahahaHAHahAHahAHa"
    )
    time.sleep(4)
    ...
    time.sleep(2)
    print(
        "Cada participante tiene una flota compuesta por 10 barcos en total. 1 barco de 4 casillas, 2 barcos de 3 casillas, 3 barcos de 2 casillas y 4 barcos de 1 casilla"
    )
    time.sleep(4)
    print("Que hacen un total de 20 vidas")
    time.sleep(2)
    print(
        "El primero que consiga destruir todos los barcos del enemigo y lo deje sin vidas, ganará la batalla"
    )
    time.sleep(3)
    ...
    for eslora in eslora_barcos:
        while True:
            ...
            if orientacion == "horizontal":
                if columna + eslora <= 10 and all(
                    tablero[fila, i] == " " for i in range(columna, columna + eslora)
                ):
                    for i in range(columna, columna + eslora):
                        tablero[fila, i] = str(eslora)
                    break
            else:
                if fila + eslora <= 10 and all(
                    tablero[i, columna] == " " for i in range(fila, fila + eslora)
                ):
...
def disparo_jugador(
    ...
    while vidas_jugador > 0 and vidas_maquina > 0:
        ...
        frase = random.choice(
            [
                "Observa el mar y dime las coordenadas donde quieres disparar grumete, confía en la fuerza....",
                "Vamos!! donde crees que se han escondido esos granujas!",
                "Grita las coordenadas y hunde sus barcos!",
            ]
        )
        print(frase)
        print("                                     ")
        ...
        columna = int(input("Ahora mete el número de la columna entre 0 y 9: "))
        print(
        ...
        if 0 <= fila < 10 and 0 <= columna < 10:  # comprueba que los inputs estén bien
            ...
            # comprueba si las coordenadas coinciden con un digito
                ...
                print("                                     ")
                vidas_maquina -= (
                    1  # resta 1 y añade modificacion a la lista de vidas de la maquina
                )
                print(f"A la banda de los TA's les quedan {vidas_maquina} vidas")
    ...
    if vidas_jugador == 0:  # fin del juego si las vidas llegan a 0
        ...
        time.sleep(1)
        print(
            "GRUMETE!!!! Tengo una noticia estupenda, no quedan barcos en el horizonte!!"
        )
        time.sleep(1)
...
def turno_maquina(tablero, vidas_jugador):
    ...
    while vidas_jugador > 0:
        ...
        fila = random.randint(0, 9)
        columna = random.randint(0, 9)
        print(f"Los malvados TA's han disparo en la fila {fila}, columna {columna}")
        time.sleep(1.5)
        print("                                     ")
